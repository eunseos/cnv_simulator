...
def reassign_overflow(initial_alloc, group_max_reads):
    ...
    return n_clone_additional_reads_per_group


def get_sample_read_counts(profile_row, new_group_cell_dict, baseline_cell_reads_dict,
                           clone_cell_barcodes, group_bam_dict, allow_dup, NCORES = 32):
    """
    ...
    if n_combined_max_reads < n_clone_additional_reads:
        logger.info(f"Total reads in all groups {n_combined_max_reads} is less than the number of additional reads to sample {n_clone_additional_reads}. Sampling with replacement.")
    ...
    if allow_dup == False:
        n_clone_additional_reads_per_group = reassign_overflow(initial_alloc, group_max_reads)
    else:
        n_clone_additional_reads_per_group = initial_alloc
    ...
    # Calculate the fraction of reads to sample from each group, allowing for duplicate sampling
    frac_reads_per_group = {}
    for group, reads in n_clone_additional_reads_per_group.items():
        max_reads = group_max_reads[group]
        if max_reads > 0:
            frac_reads_per_group[group] = []
            quotient = reads // max_reads
            remainder = reads % max_reads
            if quotient > 0:
                # If quotient > 0, then add 1.0 for each repeated sampling of entire group
                frac_reads_per_group[group].extend([1.0] * quotient)
            if remainder > 0:
                # Sample the remainder fraction of reads
                remainder_frac = remainder / max_reads
                if np.isnan(remainder_frac):
                    logger.debug(f"Trying to pick {reads} from {group} with {max_reads} reads. Setting fraction to 0.")
                    remainder_frac = 0.0
                frac_reads_per_group[group].append(remainder_frac)
        else:
...
def add_reads_to_baseline_bam(baseline_bam_path, group_bam_dict, 
                              profile_row, profile_row_index, clone_row, allow_dup,
                              TMP_BAMDIR, NCORES = 32):
    logger = logging.getLogger(__name__)
    ...
    n_baseline_reads, n_additional_reads, n_additional_reads_per_group, frac_reads_per_group = get_sample_read_counts(
        profile_row, new_group_cell_dict, baseline_cell_reads_dict, clone_cell_barcodes, group_bam_dict, allow_dup)
    ...
    for group, frac_reads in frac_reads_per_group.items():
        logger.info(f"Sampling {len(frac_reads)} times from group {group}")
        region_str = f"{profile_row.chr}:{profile_row.start}-{profile_row.end}"

        for i, frac in enumerate(frac_reads):
            logger.info(f"Sampling {frac} reads from group {group} for region {region_str}")
            sampled_bam_path = f"{TMP_BAMDIR}/row_{profile_row_index}_{profile_row.chr}_{profile_row.start}_{profile_row.end}_{group}_tmp_{i}.bam"
            if frac > 0:
                samtools_sample_reads(group_bam_dict[group], sampled_bam_path, frac,
                ...
                logger.info(f"Fraction of reads to sample from group {group} is 0. Skipping.")

    # If no temporary bam files were created (because all fractions were 0), return None
    if len(tmp_group_bam_paths) == 0:
        return None, 0

    # Merge the temporary bam files into a single bam file
    merged_unsorted_bam_path = f"{TMP_BAMDIR}/row_{profile_row_index}_{profile_row.chr}_{profile_row.start}_{profile_row.end}_merged_unsorted_tmp.bam"
    ...
    if len(tmp_group_bam_paths) < 2:
        ...
        shutil.move(tmp_group_bam_paths[0], merged_unsorted_bam_path)
        samtools_sort(merged_unsorted_bam_path, merged_bam_path, NCORES = NCORES)
    else:
        samtools_merge(tmp_group_bam_paths, merged_unsorted_bam_path, NCORES = NCORES)
        samtools_sort(merged_unsorted_bam_path, merged_bam_path, NCORES = NCORES)
        logger.info(f"Merged BAM file created: {merged_bam_path}")
    # Remove the unsorted merged bam file
    os.remove(merged_unsorted_bam_path)
    logger.info(f"Unsorted merged BAM file deleted: {merged_unsorted_bam_path}")
    ...
    unsorted_out_bam_path = f"{TMP_BAMDIR}/row_{profile_row_index}_{profile_row.chr}_{profile_row.start}_{profile_row.end}_tmp_unsorted.bam"
    out_bam_path = f"{TMP_BAMDIR}/row_{profile_row_index}_{profile_row.chr}_{profile_row.start}_{profile_row.end}_tmp.bam"
    # Merge the renamed bam file with the baseline bam file
    samtools_merge([baseline_clone_path, renamed_bam_path], unsorted_out_bam_path, NCORES = NCORES)
    samtools_sort(unsorted_out_bam_path, out_bam_path, NCORES = NCORES)
    logger.info(f"Final BAM file created: {out_bam_path}")
    # Remove the unsorted bam file
    os.remove(unsorted_out_bam_path)
    logger.info(f"Unsorted BAM file deleted: {unsorted_out_bam_path}")
...
def sample_cnv_reads(profile_dir, profile_name, group_name, allow_dup, delete_tmp, NCORES = 32):
    logger = logging.getLogger(__name__)
    ...
    os.makedirs(TMP_BAMDIR, exist_ok=False)

    try:
        cnv_profile_df = profile_df[(profile_df['chr'] != 0)]
        ...
        for index, row in cnv_profile_df.iterrows():
            ...
            if cur_copy_number == 0:
                ...
                tmp_bam_path, readcount = add_reads_to_baseline_bam(baseline_bam_path, group_bam_dict, row, index, clone_row, allow_dup, TMP_BAMDIR)
            if tmp_bam_path is None:
                logger.info(f"No reads sampled for row {index}. Skipping.")
                continue
            else:
        ...
        if os.path.exists(merged_bam_path):
            raise ValueError(f"Merged BAM file already exists: {merged_bam_path}. Please delete it before running the script again.")
        if len(all_tmp_bam_paths) < 2:
            ...
            samtools_index(merged_bam_path, NCORES = NCORES)
        ...
        if os.path.exists(final_bam_path):
            raise ValueError(f"Final BAM file already exists: {final_bam_path}. Please delete it before running the script again.")
        ...
        if os.path.exists(sorted_bam_path):
            raise ValueError(f"Sorted BAM file already exists: {sorted_bam_path}. Please delete it before running the script again.")
        samtools_sort(final_bam_path, sorted_bam_path, NCORES = NCORES)

        # Index the final bam file
        logger.info(f"Indexing final BAM file: {sorted_bam_path}")
        if os.path.exists(sorted_bam_path):
            logger.info(f"Index already exists, re-indexing: {sorted_bam_path}.")
        samtools_index(sorted_bam_path, NCORES = NCORES)

        logger.info("DONE!")
    finally:
        if os.path.exists(TMP_BAMDIR) and delete_tmp:
            shutil.rmtree(TMP_BAMDIR)
            logger.info(f"Temporary BAM directory deleted: {TMP_BAMDIR}")
        else:
            logger.info(f"Temporary BAM directory not deleted: {TMP_BAMDIR}. Set --delete_tmp to delete it.")
    return
...
def parse_args():
    ...
    parser.add_argument("--allow_dup", type=bool, default=True, help="Allow duplicate reads in the output BAM file.")
    parser.add_argument("--delete_tmp", action="store_true", help="Delete temporary BAM files after processing.")
...
def main():
    ...
    sample_cnv_reads(profile_dir, profile_name, group_name, allow_dup, delete_tmp, NCORES = NCORES)
