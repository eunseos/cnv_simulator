...
def pick_cnv_cells(available_cells, num_new_cells, region = None, NCORES = 32):
    group_bam_paths = {}
    for group in available_cells["sample_group"].unique():
        if group.isdigit():
            bam_path = f"{DATADIR}/normal_cell_bams/group_{group}_merged.bam"
        else:
            bam_path = f"{DATADIR}/normal_cell_bams/{group}.bam"
        if not os.path.exists(bam_path):
            raise ValueError(f"BAM file does not exist: {bam_path}")
        group_bam_paths[group] = bam_path
        
    # Sample cells from the available cells
    sampled_cells = available_cells.sample(n = num_new_cells, replace = False)
    sampled_cells = sampled_cells.sort_values(by = ['sample_group', 'cell_barcode'],
                                              ignore_index = True)

    for group in sampled_cells["sample_group"].unique():
        group_cell_barcodes = sampled_cells[sampled_cells["sample_group"] == group]["cell_barcode"].values
        if region:
            num_reads = samtools_get_indexed_read_count(group_bam_paths[group], region,
                                                        cell_barcodes = group_cell_barcodes,
                                                        NCORES = NCORES)
...
def assign_cell_clones(all_normal_cells, num_clones, cnv_profile, cells_per_clone_lst, group_name = None):
    ...
    for clone in range(len(cells_per_clone_lst)):
        ...
        base_cell_assign_df = pd.concat([base_cell_assign_df, pd.DataFrame([clone_row])], ignore_index=True)
    ...
    for index, row in cnv_profile.iterrows():
        if row['state'] <= 0:
            clone_cell_barcodes = base_cell_assign_df[base_cell_assign_df["clone"] == row["clone"]]["cell_barcode"].values[0]
            clone_cell_groups = base_cell_assign_df[base_cell_assign_df["clone"] == row["clone"]]["cell_group"].values[0]
        else:
            num_new_cells = int((row['copy_number']) * cells_per_clone_lst[int(row['clone'])])
            ...
            new_clone_cells = pick_cnv_cells(group_cells[~group_cells["cell_id"].isin(used_cells)], num_new_cells,
...
if __name__ == "__main__":
    ...
    logger.info(f"Input file {input_path} read successfully.")

    logger.info(f"Generating CNV profiles for {len(profile_input)} input rows.")
    logger.info(f"CNV profile name: {cnv_profile_name}")
